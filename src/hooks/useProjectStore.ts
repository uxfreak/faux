...
const generateSampleProjects = (): Project[] => {
  ...
  return names.map((name, index) => ({
    ...
    path: `/Users/user/faux-projects/${name.toLowerCase().replace(/\s+/g, '-')}`,
        ...
        if (isElectron) {
          ...
          if (saved) {
            ...
            const projects = parsed.projects.map((p: any) => ({
              ...
              path: p.path || `/Users/user/faux-projects/${p.name.toLowerCase().replace(/\s+/g, '-')}`
            }));
    ...
    try {
      if (isElectron) {
        ...
        const newProject: Project = {
          ...project,
          id: `project-${Date.now()}`,  
          path: project.path || `/Users/user/faux-projects/${project.name.toLowerCase().replace(/\s+/g, '-')}`,
          createdAt: new Date(),
          updatedAt: new Date()
        };
        setStore(prev => ({
